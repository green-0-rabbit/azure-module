resource "azurerm_private_endpoint" "acr_pe" {
  ...
  dynamic "ip_configuration" {
    ...
    content {
      name               = "${local.pep_prefix}-pep-ip-configuration"
      private_ip_address = local.networking.pe_ip
      subresource_name   = local.pep_subresource_name
      member_name        = local.pep_subresource_name
    }
  }

  # Associate the PE with the DNS zone only when DNS registration is enabled.
