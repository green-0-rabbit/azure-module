...
resource "azurerm_network_security_group" "nsg" {
  ...
  dynamic "security_rule" {
    ...
    content {
      ...
      destination_port_range     = length(security_rule.value.destination_port_ranges) > 0 ? null : security_rule.value.destination_port_range
      destination_port_ranges    = length(security_rule.value.destination_port_ranges) > 0 ? security_rule.value.destination_port_ranges : null
...
#-----------------------------------------------
locals {
  remote_vnet_name_from_id = try(element(split("/", var.remote_virtual_network_id), 8), var.remote_virtual_network_name)
}

resource "azurerm_virtual_network_peering" "vnet_to_remote_vnet" {
  count                        = var.enable_peering ? 1 : 0
  name                         = lower("peering-to-remote-${local.remote_vnet_name_from_id}")
  resource_group_name          = var.resource_group_name
  virtual_network_name         = azurerm_virtual_network.vnet.name
...
resource "azurerm_virtual_network_peering" "remote_vnet_to_vnet" {
  count                        = var.enable_peering ? 1 : 0
  name                         = lower("peering-${local.remote_vnet_name_from_id}-to-vnet-${var.vnet_name}")
...
locals {
  fw_application_rules = var.firewall != null ? { for idx, rule in var.firewall.firewall_application_rules : format("%03d-%s", idx, rule.name) => { idx = idx, rule = rule } } : {}
  fw_network_rules     = var.firewall != null ? { for idx, rule in var.firewall.firewall_network_rules : format("%03d-%s", idx, rule.name) => { idx = idx, rule = rule } } : {}
  fw_nat_rules         = var.firewall != null ? { for idx, rule in var.firewall.firewall_nat_rules : format("%03d-%s", idx, rule.name) => { idx = idx, rule = rule } } : {}
}
