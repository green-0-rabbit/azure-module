locals {
  ...
  queue_pep_subresource_name    = "queue"
  pep_member_name               = "default"
  pep_ip_config_name            = "sa-pe-ip-config"
  queue_pep_ip_config_name      = "sa-queue-pe-ip-config"
  pep_prefix                    = "sa"
  enable_private_endpoint       = var.networking != null && var.dns != null
  enable_queue_private_endpoint = local.enable_private_endpoint && try(local.dns.queue_dns_id, null) != null
