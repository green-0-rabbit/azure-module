...
variable "remote_virtual_network_id" {
  ...
  validation {
    condition     = !var.enable_peering || length(trimspace(var.remote_virtual_network_id)) > 0
    error_message = "remote_virtual_network_id must be provided when enable_peering is true"
  }
}
...
variable "remote_virtual_network_name" {
  ...
  validation {
    condition     = !var.enable_peering || length(trimspace(var.remote_virtual_network_name)) > 0
    error_message = "remote_virtual_network_name must be provided when enable_peering is true"
  }
}
